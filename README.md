...
- When you create a PR to branch "release/v1.0.0", test will run automatically
- When you merge the PR to branch "release/v1.0.0", test will run automatically again
- You will see red/green on the pipeline
- You will get email if pipeline run failed/is succesful
- If merge to main branch `release/v1.0.0` succesful, Docker image is build and pushed to DockerHub.


# Download latest Docker image of solution (backend+frontend)
Run:
```
